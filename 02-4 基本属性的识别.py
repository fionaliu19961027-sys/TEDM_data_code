...
class AttributeAnalyzer:
    def __init__(self, fast_mode=False):
        self.review_df = None
        self.attribute_categories = {}  # 存储8个属性类别
        self.feature_matrix = None
        self.shap_values = None
        self.category_stats = {}  # 存储每个类别的统计信息
    ...
    def calculate_composite_scores(self, alpha=0.4, beta=0.6):
        ...
        for category, stats in self.category_stats.items():
            ...
            self.category_stats[category]['composite_score'] = float(composite_score)
            composite_scores[category] = composite_score

        return composite_scores

    def identify_basic_categories(self, threshold_percentile=80):
        """识别基础属性类别"""
        print("识别基础属性类别...")
        ...
        composite_scores = [stats['composite_score'] for stats in self.category_stats.values()]
        threshold = np.percentile(composite_scores, threshold_percentile) if composite_scores else 0

        basic_categories = []
        other_categories = []

        for category, stats in self.category_stats.items():
            if stats['composite_score'] >= threshold:
    ...
    def visualize_results(self, basic_categories, other_categories):
        ...
        if len(viz_df) > 1:
            try:
                ...
                viz_df_sorted = viz_df.sort_values('composite_score', ascending=False)
                colors = ['red' if x else 'blue' for x in viz_df_sorted['is_basic']]
                plt.bar(viz_df_sorted['category'], viz_df_sorted['composite_score'], color=colors)
                plt.xlabel('Category')
                plt.ylabel('Composite Score')
                plt.title('Composite Scores by Category')
    ...
    def save_results(self, basic_categories, other_categories, viz_df):
        ...
        # 保存基础类别列表
        if basic_categories:
            basic_cats_df = detailed_df[detailed_df['is_basic']].sort_values('composite_score', ascending=False)
            basic_cats_df.to_excel('basic_categories.xlsx', index=False)

        # 保存其他类别列表
        if other_categories:
            other_cats_df = detailed_df[~detailed_df['is_basic']].sort_values('composite_score', ascending=False)
            other_cats_df.to_excel('other_categories.xlsx', index=False)
    ...
    def run_analysis(self, alpha=0.4, beta=0.6, threshold_percentile=80):
        ...
        if shap_scores is None:
            ...
            # 5. 计算综合得分
            self.calculate_composite_scores(alpha, beta)

        # 6. 识别基础属性类别
        basic_categories, other_categories, threshold = self.identify_basic_categories(threshold_percentile)
...
def main():
    ...
    if basic_cats is not None:
        ...
        if basic_cats:
            print(f"\n基础属性类别列表 (按综合得分排序):")
            for category in basic_cats:
                stats = analyzer.category_stats[category]
                print(f"  {category}: 综合得分={stats['composite_score']:.3f}, "
