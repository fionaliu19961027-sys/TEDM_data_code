# -*- coding: utf-8 -*-
"""
Two-stage consumer rating model (TEDM base model, without CatBoost residual correction)
- Neural network with two-stage decision structure and Huber loss
...
# -----------------------------
# 4) Two-stage NN Model with Huber loss
# -----------------------------
class TwoStageModel(nn.Module):
    ...
    def forward(self, x, prefs, strictness, utility_type, ctrl):
        ...
        # ----- Stage 2: Compensatory (only if not veto) -----
...
fold_metrics = []  # (train_mse, train_rmse, train_mae, train_r2, val_mse, val_rmse, val_mae, val_r2)
...
for train_idx, test_idx in kf.split(X_attr):
    ...
    torch.manual_seed(SEED)
    model = TwoStageModel(len(basic_attrs), len(other_attrs),
                          PREFS_tr.shape[1], CTRL_tr.shape[1]).to(DEVICE)
    ...
    # ---- Evaluation: NN only (TEDM 基础模型) ----
    model.eval()
    with torch.no_grad():
        y_tr_pred = model(X_tr_t, PREFS_tr_t, STR_tr_t, UTYPE_tr_t, CTRL_tr_t).cpu().numpy()
        y_te_pred = model(X_te_t, PREFS_te_t, STR_te_t, UTYPE_te_t, CTRL_te_t).cpu().numpy()
    ...
    tr_mse, tr_rmse, tr_mae, tr_r2 = report(f"训练集 (TEDM base) | Fold {fold_id}", Y_tr, y_tr_pred)
    te_mse, te_rmse, te_mae, te_r2 = report(f"验证集 (TEDM base) | Fold {fold_id}", Y_te, y_te_pred)

    fold_metrics.append((tr_mse, tr_rmse, tr_mae, tr_r2,
                         te_mse, te_rmse, te_mae, te_r2))

    # 可选：打印关键参数，便于和主模型对比
    print("\n=== NN 参数摘要（Fold {}）===".format(fold_id))
    with torch.no_grad():
        print("strict_thresh:", model._sigmoid_bound(model.strict_thresh_raw, 0, 1).item())
        print("mu0:", model._sigmoid_bound(model.mu0_raw, 1, 3).item())
        print("kappa:", model._sigmoid_bound(model.kappa_raw, 0, 2).item())
        print("eta:", model._sigmoid_bound(model.eta_raw, 2.5, 4).item())

    fold_id += 1

# -----------------------------
# 6) Summary of 5-fold CV
# -----------------------------
metrics_arr = np.array(fold_metrics)
means = metrics_arr.mean(axis=0)
stds  = metrics_arr.std(axis=0)

print("\n================ 5-Fold Summary (TEDM base) ================\n")
